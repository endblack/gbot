...
/******** OWNER NUMBER**********/
const ownerNumber = ["559184035474@s.whatsapp.net","559192136241@s.whatsapp.net"] 
/************************************/
...
client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\SBem vindo/a ao grupo *${mdata.subject}* comportasse para não ganha ban,`
				...
				teks = `Adeus @${num.split('@')[0]} vai como o diabo e um beijinho no cu`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
          ...
          const checkLimit = (sender) => {
           ...
           return false
			...
			switch(command) {
				...
				if ( checkATMuser(sender) <= total) return reply(`desculpe, seus pontos não é suficiente. por favor colete e tente mais tarde`)
				if ( checkATMuser(sender) >= total ) {
					confirmATM(sender, total)
					bayarLimit(sender, payout)
					await reply(`*「 SUCESSO 」*\n\n*remetente* : Admin\n*receptor* : ${pushname}\n*compra nominal* : ${payout} \n *limite de preço* : ${koinPerlimit}/limite\n *O resto de seus pontos* : ${checkATMuser(sender)}\n\nprocesso bem sucedido com número de pagamento\n${createSerial(15)}`)
				} 
				break
				case 'limit':
				   if (!isRegistered) return reply(ind.noregis())
                ...
                case 'quotes':
					...
					if (!isRegistered) return reply(ind.noregis())
					if (isLimit(sender)) return reply(ind.limitend(pusname))
                    if (args.length < 1) return reply(`insira números\nexemplo : ${prefix}infonomor 0812345678`)
				...
				case 'bcgc':
                   ...
                   if (isLimit(sender)) return reply(ind.limitend(pusname))
					...
					if (!isRegistered) return reply(ind.noregis())
					client.updatePresence(from, Presence.composing) 
					teks = `\`\`\`Esta é uma lista de grupos XPTN BOT :\n\n\`\`\``
					no = 0
					for (let hehehe of groupId) {
						no += 1
						teks += `\`\`\`[${no.toString()}]\`\`\` @${hehehe.split('@')[0]}\n`
					}
					teks += `\n\`\`\`Total de grupos : ${groupId.length}\`\`\``
            	...
            	case 'mining':
                      ...
                      if (!isEventon) return reply(`Desculpe ${pushname} o evento de mineração não foi ativado pelo proprietário`)
                      if (isOwner) {
                      const one = 999999999
          ...
          if (!isRegistered) return reply(ind.noregis())
			...
			if (isLimit(sender)) return reply(ind.limitend(pusname))
                ...
                if (!isRegistered) return reply(ind.noregis())
                if (isLimit(sender)) return reply(ind.limitend(pusname))
					if (args.length < 1) return reply('O que você quer procurar, humm🤔?')
				...
				if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply('Envie fotos ou tags com lengenda ${prefix}𝗼𝗰𝗿')
					}
					await limitAdd(sender)
					break
				case 'stiker': 
				...
				case 's':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						reply(`Envie fotos ou tags com lengenda ${prefix}sticker`)
					}
					break
				case 'tts':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				if (args.length < 1) return client.sendMessage(from, 'Qual do codigo do idioma cara?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Qual texto você está criando tio?  é minha voz: v?', text, {quoted: mek})
					dtt = body.slice(8)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 300
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(ind.ownerb())
					prefix = args[0]
					reply(`O novo prefixo agora é : ${prefix}`)
					break 
				case 'tiktokstalk':
				if (!isRegistered) return reply(ind.noregis())
				if (isLimit(sender)) return reply(ind.limitend(pusname))
				try {
						...
						console.log(`Error :`, color(e,'red'))
						reply('[𝗘𝗥𝗥𝗢𝗥] o username é inválido')
					}
					await limitAdd(sender)
					break
                 case 'linkgc':
				...
				if (!isGroup) return reply(ind.groupo())
					if (!isOwner) return reply(ind.ownerb())
				await reply(from, 'Adeus').then(() => client.leaveGroup(groupId))
					break
				case 'bc': 
					if (!isOwner) return reply(ind.ownerb()) 
					if (args.length < 1) return reply('.......')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `❮●BOT_MEGAH●❯\n\n${body.slice(4)}`})
						}
			   	...
			   	case 'setpp': 
                       ...
                       media = await client.downloadAndSaveMediaMessage(mek)
                        ...
                        reply('Icone do grupo alterado')
					break						
				case 'add':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args.length < 1) return reply('Quem você quer adicionar, um gênio?')
					...
					try {
						...
						console.log('Error :', e)
						reply('Não foi possivel adicionar o cara 🥴')
					}
					break
					case 'grupo':
					case 'group':
					...
					if (args[0] === 'abrir') {
						...
						reply(`*GRUPO FECHADO, SOMETE ADM MANDA MENSAGENS AGORA*`)
            ...
            case 'owner':
            case 'creator':
                  client.sendMessage(from, {displayname: "MAX", vcard: vcard}, MessageType.contact, { quoted: mek})
                  client.sendMessage(from, 'Esse é o contato do meu adm, se for pra falar merda nem chama!',MessageType.text, { quoted: mek} )
					break    
           case 'setname':
                if (!isGroup) return reply(ind.groupo())
			    if (!isGroupAdmins) return reply(ind.admin())
                ...
                client.sendMessage(from, 'Descrição do grupo alterada com sucesso!)', text, {quoted: mek})
					break
           case 'demote':
					if (!isGroup) return reply(ind.groupo())
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o cara que vc quer rebaixa!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Isso aconteceu por que vc não mamou os adm 😐 :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`haha ${mentioned[0].split('@')[0]} isso aconteceu por que vc não mamou os adm 🙃`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
				case 'promote':
					...
					if (!isGroupAdmins) return reply(ind.admin())
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('Marque o cara que vc quer promover!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = ''
						for (let _ of mentioned) {
							teks += `Parabéns por se torna um adm, ver se mama os adm certinho ai hein (+_+) :\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					} else {
						mentions(`Parabéns por se torna um adm @${mentioned[0].split('@')[0]}, ver se mama os adm certinho hein (+_+)`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break	
			     	case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Toma esse ban @${mentioned[0].split('@')[0]} 🏃`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmin':
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o simi neste grupo')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./database/bot/simi.json', JSON.stringify(samih))
						reply('️️Desativou com sucesso o simi neste grupo')
					} else {
						reply(ind.satukos())
					}
					break
				case 'nsfw':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Ativou com sucesso o nsfw neste grupo')
					} else if (Number(args[0]) === 0) {
						nsfw.splice(from, 1)
						fs.writeFileSync('./database/bot/nsfw.json', JSON.stringify(nsfw))
						reply('Desativou com sucesso o nsfw neste grupo')
					} else {
						reply(ind.satukos())
					}
					break
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('Boas vindas ativada neste grupo')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('Boas vindas desativada')
					} else {
						reply(ind.satukos())
					}
					break
                 case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*️EVENT está ativo no grupo aparti de agora*')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/event.json', JSON.stringify(event))
						reply('*EVENT desativado*')
					} else {
						reply(ind.satukos())
					}
					break
				case 'clone':
					...
					if (args.length < 1) return reply(' *Quem vc deseja clonar?* ')
					...
					try {
						...
						mentions(`Foto do perfil atualizada com sucesso usando a foto de perfil deste sujeito @${id.split('@')[0]}`, [jid], true)
					} catch (e) {
						reply(ind.stikga())
					}
					await limitAdd(sender)
